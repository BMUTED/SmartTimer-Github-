...
public class AFK_Checker : MonoBehaviour
{
    ...
    void AFKTimerFlowing()
    {
        if (!IsInputDetected) //사용자가 잠수 상태일때
        {
            ...
            if (IdleTimer >= GameManager.Instance.SaveManagerSC.SaveData.DefaultPeriodTime) //DefaultPeriodTime 초 이상 잠수 상태인 경우
            {
                IdleTimer = 0; //잠수 타이머 초기화
                SetIsTimerStopBeingAFK(true);
                Debug.Log($"{GameManager.Instance.SaveManagerSC.SaveData.DefaultPeriodTime} 동안, 입력이 없어서, 타이머가, 잠수 상태라 판단되었습니다");
            }
        }
